...
int generate_test_vectors()
{
	...
	unsigned char diff, diff1, diff2, list[number] = {0}, temp, temp1;
	...
	int token[4] = {0x1, 0x2, 0x4, 0x8};
	int token1[11] = {0x3, 0x5, 0x6, 0x7, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf};
	...
	for( itr = 0; itr < 1000; ++itr) {
		...
		diff = rand() % 4;//induced bit fault at the lsb position
		temp = diff;
		diff = token[ diff ];
		temp1 = 0xf;
		...
		for( i1 = 0; i1 < 1000; ++i1 ) {
			...
			/*if( diff2 == 0 )
				diff2 = rand() & 0xf;*/
			state[ pos / 8 ][ pos % 8 ] = token1[diff2];
			...
			if ( fault_on_crypto_aead_decrypt(msg2, &mlen2, NULL, ct1, clen, ad, adlen, nonce, key, diff, pos ) == 0 ) {
				...
				//printf("input diff = %d, Number of forgeries is %d\n",diff, count);
