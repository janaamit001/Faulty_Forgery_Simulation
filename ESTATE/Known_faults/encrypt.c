...
/*************************************************************************************************************/
static void faulty_fcbc_star(u8 *tag, const u8 (*round_keys)[16], const u8 *in, const u64 inlen, const u64 in_blocks, const u8 *twks, unsigned char diff, unsigned char pos)
{
	u8 zero = 0x00;
	u8 temp[CRYPTO_BLOCKBYTES];
	...
	if(PARTIAL_BLOCK_LEN(in_blocks,inlen) != CRYPTO_BLOCKBYTES)
	{
		...
		memcpy_and_zero_one_pad(temp, &in[(in_blocks-1)*CRYPTO_BLOCKBYTES], PARTIAL_BLOCK_LEN(in_blocks, inlen));
		xor_bytes(temp, tag, CRYPTO_BLOCKBYTES);
		faulty_tweaes_enc(tag, &round_keys[0], &twks[1], temp, diff, pos );
	}
	else
	{
		// compute T = E^2/4/6(K,T+I_{i-1})
		memcpy(temp, tag, CRYPTO_BLOCKBYTES);
		xor_bytes(temp, &in[(in_blocks-1)*CRYPTO_BLOCKBYTES], CRYPTO_BLOCKBYTES);
		faulty_tweaes_enc(tag, &round_keys[0], &twks[0], temp, diff, pos );
	}
}



static void faulty_mac(u8 *tag, const u8 (*round_keys)[16], const u8 *nonce, const u8 *ad, const u64 adlen, const u64 ad_blocks, const u8 *pt, const u64 ptlen, const u64 pt_blocks, unsigned char diff, unsigned char pos)
{
	u8 twks[2] = { 0 };
	u8 temp[CRYPTO_NPUBBYTES];
	memcpy(temp, nonce, CRYPTO_BLOCKBYTES);
	...
	if(adlen > 0)
	{
		...
		twks[0] = ptlen != 0 ? 0x02 : 0x06;
		twks[1] = ptlen != 0 ? 0x03 : 0x07;
		faulty_fcbc_star(&tag[0], &round_keys[0], ad, adlen, ad_blocks, &twks[0], diff, pos );
	}
...
)
{
	...
	// generate tag
	u64 pt_blocks = *ptlen%CRYPTO_BLOCKBYTES ? ((*ptlen/CRYPTO_BLOCKBYTES)+1) : (*ptlen/CRYPTO_BLOCKBYTES);
	faulty_mac(&tag[0], &round_keys[0], npub, ad, adlen, ad_blocks, pt, *ptlen, pt_blocks, diff, pos );
